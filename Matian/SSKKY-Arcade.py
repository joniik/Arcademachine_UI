...
atexit.register(save_game_stats)
...
def handle_key_event(key):
    ...
    if key.keytype == Key.AXIS:
        ...
        if key.value == 1:
            if game_open == False:
                game_name_label.configure(text="Loading...", font=("Arcade Normal", 25),)
                print("play")
                open_files()
                update_game_stats(x)
...
def update_game_stats(index):
    global game_stats
    game_name = os.path.splitext(os.path.basename(folder_files_list[index][0]))[0]

    # Update play count for the game
    if game_name in game_stats:
        game_stats[game_name] += 1
    else:
        game_stats[game_name] = 1
...
def update_display(image_number):
    ...
    # Remove the .exe extension from the current_folder_name
    game_name_without_extension = os.path.splitext(current_folder_name)[0]

    my_label.configure(image=image_list[image_number])
    if game_open == False:
        game_name_label.configure(text=game_name_without_extension)

def update_display_loop():
    update_display(x)
    app.after(1000, update_display_loop)  # Schedule the next update in 1 second
...
def tkinter_loop():
    ...
    game_name_without_extension = os.path.splitext(current_folder_name)[0]
    game_name_label.configure(text=game_name_without_extension, font=("Arcade Normal", 25),)
...
def check_game_open_periodically():
    global game_open
...
def main():
    ...
    # Start a thread to check if the game is open periodically
    game_check_thread = threading.Thread(target=check_game_open_periodically)
    game_check_thread.start()

    tkinter_loop()

if __name__ == "__main__":
    if os.path.isfile("game_stats.txt"):
        with open("game_stats.txt", "r") as f:
            for line in f:
                parts = line.strip().split(": ")
                if len(parts) == 2:
                    game_stats[parts[0]] = int(parts[1])
    main()
